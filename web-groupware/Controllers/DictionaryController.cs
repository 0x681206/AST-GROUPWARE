    ...
    public class DictionaryController : BaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            try { 
                ...
                var items = await _context.T_DIC.OrderBy(item => item.dic_cd).ThenBy(item => item.dic_kb).ToListAsync();
        ...
        public async Task<IActionResult> Create(DictionaryDetail model)
        {
            try
            {
                if (_context.T_DIC.Any(d => d.dic_kb == model.dic_kb && d.dic_cd == model.dic_cd))
                {
                ...
                {
                    ModelState.AddModelError("", Common.Constants.Messages.IS_VALID_FALSE);
                    return View(model);
                }
                var model_new = new T_DIC();
        ...
        public async Task<IActionResult> Edit(string? dic_cd, int dic_kb)
        {
            try
            {
                var items = await _context.T_DIC.FirstOrDefaultAsync(m => m.dic_cd == dic_cd && m.dic_kb == dic_kb);
                if (items == null)
                {
                    return RedirectToAction("Index");
                }
        ...
        public async Task<IActionResult> Edit(DictionaryDetail model)
        {
            try
            {
                ...
                var record = await _context.T_DIC.FirstOrDefaultAsync(x => x.dic_cd == model.dic_cd && x.dic_kb == model.dic_kb);
                if (record == null)
                {
