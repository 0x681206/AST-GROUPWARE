...
namespace web_groupware.Controllers
{
    public class LoginController : BaseController
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                string mail = HttpContext.User.FindFirstValue(Utilities.ClaimTypes.MAIL);
                string staf_cd = HttpContext.User.FindFirstValue(Utilities.ClaimTypes.STAF_CD);
                if (mail == null)
                {
                    return View(model);
                }
                var user_in_db = await _context.T_STAFFM.FirstOrDefaultAsync(m => m.mail == mail);
                if (user_in_db == null)
                {
                    return View(model);
                }

                if (user_in_db.staf_cd.ToString() == staf_cd)
                {
        ...
        public async Task<IActionResult> Index([Bind("mail, password, remember")] LoginViewModel model)
        {
            try
            {
                ...
                if (user_in_db == null)
                {
                    ModelState.AddModelError("", Common.Constants.Messages.LOGIN_ERROR_MESSAGE01);
                    return View(model);
                }
